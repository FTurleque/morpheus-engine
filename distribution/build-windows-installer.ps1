...
$appImage = Join-Path $dist ".m14-windows\image\morpheus"
...
if (-not (Test-Path $appImage)) {
    Write-Host "Portable M14 app-image not found; building it first..."
...
Write-Host "Building Windows EXE installer from the validated M14 app-image..."
& $jpackage `
    --type exe `
    --name morpheus `
    --app-version $Version `
...
Write-Host "The installer contains MORPHEUS CLI/MCP/API, optional MINOS/NEXUS client adapters and the M14 read-only orchestration contract; MINOS, NEXUS and JARVIS are not bundled."
