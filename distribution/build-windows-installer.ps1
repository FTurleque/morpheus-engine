...
$appImage = Join-Path $dist ".m13-windows\image\morpheus"
...
if (-not (Test-Path $appImage)) {
    Write-Host "Portable M13 app-image not found; building it first..."
...
Write-Host "Building Windows EXE installer from the validated M13 app-image..."
& $jpackage `
    --type exe `
    --name morpheus `
    --app-version $Version `
...
Write-Host "The installer contains MORPHEUS CLI/MCP/API plus optional MINOS and NEXUS client adapters; neither external engine is bundled."
