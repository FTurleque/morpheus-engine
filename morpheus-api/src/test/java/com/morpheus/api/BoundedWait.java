 ...
 *
 * <p>The states waited on here belong to an HTTPS facade that publishes them over its own status endpoint and
 * offers no latch. There is nothing to await, so a bounded poll is not a shortcut around synchronisation --
 * it is what reading that state looks like. What was wrong with the hand-rolled copies of this loop was never
 * the polling; it was that every one of them expired saying only {@code "condition was not satisfied within
 * PT60S"}, which names nothing that was observed and leaves the reader to reconstruct by hand what the system
 * was doing. A diagnosis on one failure is worth more than ten attempts to make failure unlikely.</p>
 *
 ...
 * state sat unchanged the whole time or was still moving when the budget ran out. A state that never moved
 * points at something stuck or never started; a state still moving at the deadline points at a budget too
 * short for a loaded runner, and those two have opposite fixes.</p>
 *
 ...
 *
 * <p>This class is deliberately per-module. {@code morpheus-mcp-transport} carries its own copy because it has
 * no MORPHEUS dependencies at all, and giving it one to share a test helper would be a real architectural
 * change made for a cosmetic reason. {@code BoundedWaitOwnershipTest} keeps the two in step and stops the
 * twelve hand-rolled helpers from growing back.</p>
 */
final class BoundedWait {

    /**
     * Sampling interval for a state read over HTTPS from the facade under test.
     *
     * <p>Each sample is a real request against the very server whose occupancy is being measured, so polling
     * faster does not observe sooner -- it adds load of its own to what is being observed.</p>
     */
    static final Duration HTTPS_STATUS_POLL = Duration.ofMillis(25);

    /**
     * Sampling interval for a counter this process can read without touching the network.
     *
     * <p>Cheap to observe, so the interval only needs to stay well below the transition being watched for.</p>
     */
    static final Duration LOCAL_COUNTER_POLL = Duration.ofMillis(20);
    ...
    // observe by looking. The state belongs to an HTTPS facade that publishes its state and offers no latch,
