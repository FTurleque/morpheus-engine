...
- **Deux commandes ouvrent le store avant de vérifier leurs options.** `projects add --workspace ""` et
  ...
  le vérifie pour `projects add`. `external-references list|resolve` cherche le projet avant de lire `--owner` et
  `--reference` : pour un projet inconnu, `--owner ""` rend `4` (projet introuvable) avant `2`. CLI-8 (#383) déplace le
  contrôle de ses options avant l'accès à l'état ; ce contrôle portant maintenant le refus du vide, `2` suit sans autre
  changement.
