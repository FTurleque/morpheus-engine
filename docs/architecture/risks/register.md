...
| DT-10 | Couverture historique globale encore modeste malgré un changed-code gate strict | Qualité | **Moyenne** | Ratchets M21 actifs à `1550 / 385`, couverture `85,0% / 68,0%` agrégée et `62,0% / 53,5%` par module (mesures exact-head dual-plateforme : échelle agrégée le 09/09/2026, plus basse des quatre 85,7263% lignes / 68,5246% branches sous Linux ; échelle par module le 08/09/2026, Windows 62,53% / 53,81%, Linux 62,51% / 53,80%) ; #184 est clôturée, conserver la remontée progressive uniquement après nouvelle preuve exact-head reproductible. **Requalification du plafond par module en attente de deux plateformes (constaté le 15/09/2026)** : `PER_MODULE_QUALIFIED_*` (62,5013 % / 53,7997 %, qualifié le 08/09/2026) est dépassé par la mesure depuis — 63,02–63,06 % de lignes relevés le 09/09/2026, point de départ d'une requalification. Elle exige une mesure exact-head Windows **et** Linux citée dans le commentaire de la constante, qualifiée sur la plus basse, avec un ratchet placé sous le plafond d'une marge supérieure à la variation inter-runs (ADR-0104) ; jamais sur une seule plateforme, jamais par alignement des deux échelles. D'ici là le plafond daté borne plus bas que nécessaire, sans affaiblir aucun gate |
...
| DT-15 | Généralisation d'ADR-0103, groupe 2 : la frontière transport/JSON des routeurs `*HttpRoutes` n'est encore exprimée qu'en texte, routeur par routeur | Tests d'architecture | **Faible** | **Décidée le 11/09/2026, non livrée — une PR dédiée.** L'amendement d'ADR-0103 tranche la généralisation en trois groupes : le groupe 1 (trois interdits vrais pour les dix-sept routeurs) est livré dans `HttpRoutesFamilyArchitectureTest`, le groupe 3 (cibles sans famille) est refusé. Reste le groupe 2, **par capacité** : une règle `noClasses()` sur les huit routeurs sans corps de requête, **listés explicitement** dans le `that()` (`Composition`, `Diagnostics`, `ExternalReference`, `IntegrationStatus`, `ProviderPlugin`, `Root`, `Specifications`, `Versions`), vers `com.sun.net.httpserver..`, `tools.jackson..` et `MorpheusHttpRequestDecoder` ; puis, pour les neuf routeurs qui lisent un corps et portent légitimement `HttpExchange`, l'établissement littéral par littéral de ce qui leur reste interdit. Conditions : une preuve de violation par règle, le texte conservé partout où le type interdit déclare une constante inlinable, un compte `@Test` qui ne descend pas. **Aucune fuite connue** : c'est un gain de portée, pas une correction |
