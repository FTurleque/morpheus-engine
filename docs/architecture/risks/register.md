...
| **A-15** — le ruleset exige une PR mais pas d'approbation humaine | **décision de gouvernance, aucune modification GitHub effectuée.** Configuration conservée pour un dépôt à mainteneur unique. Si le dépôt devient collaboratif : `CODEOWNERS`, 1 approbation minimum, protection renforcée sur `.github/workflows/**`, `morpheus-api/**`, `morpheus-provider-sdk/**`, `morpheus-mcp-transport/**`, `morpheus-store-sqlite/**`, `scripts/**` |
