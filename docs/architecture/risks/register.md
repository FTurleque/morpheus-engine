...
| DT-17 | Les quatre routeurs d'extension écrivent chacun leur propre enveloppe de réponse | API HTTP | **Faible à moyenne** | **Nommée le 16/09/2026, après vérification sur le code.** DT-16 laissait ce reste non nommé faute de décision, et le supposait justifié par « des enveloppes légitimement différentes (exports bruts, en-tête `Allow`, codes métier propres) » : **la lecture des sources réfute cette justification**. `MorpheusPolicyHttpRoutes`, `MorpheusPolicyManagementHttpRoutes` et `MorpheusReasoningHttpRoutes` portent un `send`/`sendJson` **identique octet pour octet** à `MorpheusHttpResponseWriter.send` — même sérialiseur, mêmes trois en-têtes (`Content-Type`, `Cache-Control`, `X-Content-Type-Options`) dans le même ordre, même écriture ; `MorpheusQueryHttpRoutes` n'en diffère qu'en hissant le content-type en paramètre (`sendRaw`), la mécanique d'en-têtes restant identique. Les quatre redéclarent en outre `ApiSuccess`, `ApiError` et `ApiErrorEnvelope` en `private`, avec des signatures identiques aux records **`public`** déjà portés par `MorpheusHttpServer`. Les enveloppes ne sont donc pas différentes : elles sont identiques et dupliquées quatre fois — douze déclarations de types redondantes plus quatre copies d'une méthode de huit lignes. Les deux seules spécificités réelles ne s'y opposent pas : l'export brut de `Query` est un **paramètre que le writer partagé n'offre pas encore** (un manque du writer, pas une divergence d'enveloppe), et l'en-tête `Allow` est posé sur l'`exchange` **avant** l'appel d'envoi, donc orthogonal au writer. **La vraie question de conception, à trancher avant migration, est où vit l'enveloppe partagée** : sur `MorpheusHttpServer`, ou sur un type neutre que le serveur et les routeurs consomment tous deux. Le couplage n'est pas un argument contre : les quatre routeurs référencent **déjà** `MorpheusHttpServer.API`, constante inlinée par `javac` donc invisible à ArchUnit — passer par les records publics rendrait *visible* un couplage source qui existe aujourd'hui, au lieu de le créer (asymétrie ADR-0103). **Conditions d'acceptation** : la même parité de réponse que DT-16 a exigée pour la requête, épinglée **avant** migration sur le code d'origine — statut, `Content-Type`, `Allow` et enveloppe comparés octet pour octet sur les quatre contextes, succès et échecs, y compris les exports bruts de `Query` et les codes métier propres ; puis reclassement du groupe dans `HttpRoutesTransportBoundaryArchitectureTest`, dont le nom `ROUTERS_THROUGH_THE_DECODER_WRITING_THEIR_OWN_RESPONSE` perdrait sa raison d'être |
