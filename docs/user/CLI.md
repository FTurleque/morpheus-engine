...
compté par `warnings=`, dans l'ordre du résultat — puis une ligne `truncationReason=…` par raison distincte. Une analyse
complète porte toujours au moins `ACCEPTANCE_CRITERIA_UNAVAILABLE` : les critères d'acceptation ne sont pas dans le
modèle normalisé. Une traversée tronquée n'est pas un refus : le code de sortie reste `0`.
